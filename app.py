...
def api_deposit_submit():
    ...
    try:
        supa_request('POST', 'deposit_requests', {
            ...
            'amount': amount,
            'status': 'pending',
            'note': f'{method} deposit request'
        })
        # Notify admin via chat
        supa_request('POST', 'chat_messages', {
            'username': username,
            'message': f'📎 Payment slip submitted via {method}. Awaiting confirmation.',
            'sender': 'user'
        })
        return jsonify({'success': True})
    except Exception as e:
...
def admin_deposit_action():
    ...
    try:
        ...
        if action == 'approve':
            ...
            supa_request('POST', 'chat_messages', {
                'username': username,
                'message': f'✅ Payment confirmed! {int(float(amount or req.get("amount",0))):,} pts added to your account.',
                'sender': 'admin'
            })
        else:
